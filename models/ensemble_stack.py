...
class EnsembleError(Exception):
    """Base exception for ensemble operations."""
    pass

class EnsembleTrainingError(EnsembleError):
    """Exception raised when ensemble training fails."""
    pass

class EnsemblePredictionError(EnsembleError):
    """Exception raised when ensemble prediction fails."""
    pass
...
class NeuralStacker(nn.Module):
    def __init__(self, input_size: int, hidden_size: int = 64):
        try:
            ...
            raise EnsembleError(f"Failed to initialize NeuralStacker: {str(e)}")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        try:
            ...
            raise EnsembleError(f"Failed in NeuralStacker forward pass: {str(e)}")

class AdvancedEnsemble:
    def __init__(self, use_gpu: bool = True, model_manager: Optional[ModelManager] = None):
        try:
            ...
            raise EnsembleError(f"Failed to initialize AdvancedEnsemble: {str(e)}")
    ...
    def _prepare_meta_features(self, predictions_list: List[np.ndarray]) -> np.ndarray:
        ...
        try:
            ...
            raise EnsembleError(f"Failed to prepare meta-features: {str(e)}")
    ...
    def _train_neural_stacker(self, X: np.ndarray, y: np.ndarray) -> NeuralStacker:
        ...
        try:
            ...
            raise EnsembleTrainingError(f"Failed to train neural stacker: {str(e)}")
    ...
    def majority_vote(self, predictions_list: List[np.ndarray]) -> pd.Series:
        ...
        try:
            ...
            raise EnsemblePredictionError(f"Failed in majority voting: {str(e)}")
    ...
    def weighted_soft_voting(self, probabilities_list: List[np.ndarray], 
        ...
        try:
            ...
            raise EnsemblePredictionError(f"Failed in weighted soft voting: {str(e)}")
    ...
    def stacking(self, predictions_list: List[np.ndarray], 
        ...
        try:
            ...
            raise EnsembleTrainingError(f"Failed in stacking: {str(e)}")
    ...
    def dynamic_weighting(self, predictions_list: List[np.ndarray],
        ...
        try:
            ...
            raise EnsemblePredictionError(f"Failed in dynamic weighting: {str(e)}")
    ...
    def ensemble_predict(self, predictions_list: List[np.ndarray],
        ...
        try:
            ...
            logger.info(f"Ensemble prediction completed in {execution_time:.2f} seconds")
            
            return result
        except Exception as e:
            raise EnsemblePredictionError(f"Failed in ensemble prediction: {str(e)}")
...
def majority_vote(predictions_list: List[np.ndarray]) -> pd.Series:
    ...
    try:
        ...
        raise EnsemblePredictionError(f"Failed in majority vote convenience function: {str(e)}")
...
def weighted_soft_voting(probabilities_list: List[np.ndarray], 
    ...
    try:
        ...
        raise EnsemblePredictionError(f"Failed in weighted soft voting convenience function: {str(e)}")
