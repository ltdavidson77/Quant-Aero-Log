...
# debug_tests.py
# ==========================
# Helper script for running tests in debug mode.
...
def run_with_profiling(func, *args, **kwargs) -> Optional[pstats.Stats]:
    ...
    if os.environ.get('ENABLE_PROFILING') == 'true':
        ...
        try:
            ...
            stats.dump_stats(f'profile_results_{timestamp}.prof')
...
def main():
    ...
    try:
        ...
        logging.info("Starting test execution in debug mode")
        ...
        pytest_args = [
            ...
            '--tb=long',  # Show detailed traceback
            '--durations=10',  # Show 10 slowest tests
        ]
        
        # Run tests with profiling if enabled
        stats = run_with_profiling(pytest.main, pytest_args)
        ...
        if stats is None or stats == 0:
            ...
            logging.error(f"Tests failed with exit code: {stats}")
